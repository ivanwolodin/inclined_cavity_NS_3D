    ...
    for (int j = 1; j <= Ny - 1; j++) {
        ...
        u_auxilliary[0][j] =
                ...
                (dt / Re) * (
                        ...
                        (u[1][j + 1] - 2 * u[1][j] + u[1][j - 1]) * dydy2[j]
                ) -
                NONLINEAR * (
                        (dt / dx) *
                        (
...
#pragma omp parallel for
    for (int i = 1; i <= Nx - 1; i++) {
        v_auxilliary[i][0] = 0 + (dt / (dy[0])) * (p[i][1] - p[i][0]);
        ...
        for (int j = 1; j <= Ny - 1; j++) {
            u_auxilliary[i][j] =
                    ...
                    (dt / Re) * (
                            ...
                            (u[i + 1][j + 1] - 2 * u[i + 1][j] + u[i + 1][j - 1]) * dydy2[j]
                    ) -
                    NONLINEAR * (
                            (dt / dx) *
                            (
        ...
        for (int j = 1; j < Ny - 1; j++) {
            ...
            v_auxilliary[i][j] =
                    ...
                    (dt / Re) * (
                            ...
                            (v[i][j + 1] - 2 * v[i][j] + v[i][j - 1]) * dydy2[j]
                    ) -
                    NONLINEAR * (
                            (dt / dx) *
                            (
...
#pragma omp parallel for
    for (int i = 1; i <= Nx - 1; i++) {
        for (int j = 1; j < Ny - 1; j++) {
            v[i][j] = v_auxilliary[i][j] - dtdy2[j] * (p[i][j + 1] - p[i][j]);
        }
    }
